...
namespace CryptoNote {
  ...
  public:
    ...
    virtual bool findOutputsToAccount(const AccountAddress& addr, const SecretKey& viewSecretKey, std::vector<size_t>& outs, uint64_t& outputAmount) const override;
  ...
  Transaction::Transaction(const Blob& data) {
    ...
    if (
      !cryptonote::parse_and_validate_tx_from_blob(blob, transaction) ||
  ...
  Hash Transaction::getTransactionHash() const {
    return reinterpret_cast<const Hash&>(get_transaction_hash(constructFinalTransaction()));
  }

  Hash Transaction::getTransactionPrefixHash() const {
    return reinterpret_cast<const Hash&>(get_transaction_prefix_hash(constructFinalTransaction()));
  }
  ...
  void Transaction::signInputMultisignature(size_t index, const PublicKey& sourceTransactionKey, size_t outputIndex, const AccountKeys& accountKeys) {
    ...
    crypto::generate_signature(reinterpret_cast<const crypto::hash&>(getTransactionPrefixHash()),
