...
namespace CryptoNote {
  ...
  public:
    ...
    TransactionExtra(const std::vector<uint8_t>& extra) {
      if (!parse(extra))
        throw std::runtime_error("Failed to parse transaction extra field");
    }
  ...
  private:
    ...
    std::vector<cryptonote::tx_extra_field>::const_iterator find(const type_info& t) const {
      return std::find_if(fields.begin(), fields.end(), [&t](const cryptonote::tx_extra_field& f) { return t == f.type(); });
    }

    std::vector<cryptonote::tx_extra_field>::iterator find(const type_info& t) {
      return std::find_if(fields.begin(), fields.end(), [&t](const cryptonote::tx_extra_field& f) { return t == f.type(); });
    }
